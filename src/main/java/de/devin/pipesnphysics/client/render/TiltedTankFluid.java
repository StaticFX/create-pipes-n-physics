...
/**
 * Renders fluid in Create tanks on Sable sub-levels with a tilted surface, wave simulation and
 * proper wall clipping at any rotation — the fluid stays level with the WORLD as the contraption
 * rolls, instead of riding the hull the way a block-local draw does.
 *
 * Plain client code rather than a mixin body on purpose: it is reached from TWO hooks, since a tank
 * derivative may ship its own block-entity renderer that Create's never runs (see
 * {@code FluidTankRendererMixin} and {@code BlockEntityRenderDispatcherMixin}). The box is read
 * along the multiblock's OWN main axis, so a horizontal fluid vessel is not treated as an upright
 * tank of the wrong shape.
 */
public final class TiltedTankFluid {
    private TiltedTankFluid() {}

    // --- Constants ---
    private static final float INSET = 0.003f;
    private static final float CAP_HEIGHT = 1 / 4f;
    private static final float HULL_WIDTH = 1 / 16f + 1 / 128f;
    private static final float PUDDLE_HEIGHT = 1 / 16f;
    ...
    private static final float[][] FACE_DEBUG_COLORS = {
    ...
    private static void pruneStaleWaveState() {
        long now = System.currentTimeMillis();
        if (now - lastPruneMs < WAVE_PRUNE_INTERVAL_MS) return;
        lastPruneMs = now;
    ...
    private static Surface resolve(FluidTankBlockEntity be, float partialTicks, boolean advanceSmoothing) {
        ...
        // A multiblock lays out along its OWN main axis: a Create tank stacks up Y, a Create:
        // Connected fluid vessel runs along X or Z with `width` as its cross-section. Reading the
        // box as always-vertical put a horizontal vessel's fluid in a box of the wrong shape.
        Direction.Axis axis = be.getMainConnectionAxis();
        int width = acc.pipesnphysics$getWidth();
        int length = acc.pipesnphysics$getHeight();
        // The ends of the main axis are capped, the cross-section is walled by the thin hull.
        float mainMin = CAP_HEIGHT, mainMax = length - CAP_HEIGHT;
        float crossMin = HULL_WIDTH, crossMax = width - HULL_WIDTH;
        // Fill rises through the box's VERTICAL extent, less the puddle Create leaves at the floor.
        float totalHeight = (axis == Direction.Axis.Y ? mainMax - mainMin : crossMax - crossMin)
                - PUDDLE_HEIGHT;
        ...
        float xMin = (axis == Direction.Axis.X ? mainMin : crossMin) + INSET;
        float xMax = (axis == Direction.Axis.X ? mainMax : crossMax) - INSET;
        float yMin = (axis == Direction.Axis.Y ? mainMin : crossMin) + INSET;
        float yMax = (axis == Direction.Axis.Y ? mainMax : crossMax) - INSET;
        float zMin = (axis == Direction.Axis.Z ? mainMin : crossMin) + INSET;
        float zMax = (axis == Direction.Axis.Z ? mainMax : crossMax) - INSET;
        ...
        float[][] corners = buildCorners(mins, maxs);
        float[] dist = computeCornerDistances(corners, localUp, cx, cy, cz);
        int dominant = dominantAxis(nxf, nyf, nzf);
        ...
        float planeOffset = findVolumeCorrectOffset(dist, fillFraction,
    ...
    public static boolean render(FluidTankBlockEntity be, float partialTicks, PoseStack ms,
        ...
        if (intersections.size() < 3) return true;
        ...
        float[] waveCur = stepWaveSimulation(
        ...
        buildGrid(grid, bounds, plane, wavesEnabled, waveCur, gridRaw, gridOutside);
        ...
        if (!hideTexture) {
            ...
            renderSurface(qvc, mat, grid, gridRaw, gridOutside, bounds, plane, style);
            ...
            renderWallSkirts(qvc, mat, grid, gridRaw, gridOutside,
                    bounds, plane, style, light);
            renderWallFaces(qvc, mat, corners, dist, bounds, plane.dominant(), style, light);
        }
        ...
        if (PipesNPhysicsConfig.FLUID_DEBUG_RENDER.get()) {
            ...
            if (hideTexture) {
                ...
                renderDebugWireframe(lvc, mat, grid, gridRaw, gridOutside,
                ...
                renderDebug(lvc, mat, grid, gridClamped, gridOutside, corners, dist);
            }
        }
        return true;
    }
     ...
     */
    private static float[][] buildCorners(float[] mins, float[] maxs) {
        return new float[][] {
     ...
     */
    private static float[] computeCornerDistances(float[][] corners, Vector3d localUp,
     ...
     */
    private static float findVolumeCorrectOffset(float[] dist, float fillFraction,
        ...
        for (int iter = 0; iter < 16; iter++) {
            float mid = (lo + hi) / 2f;
            float vol = analyticalVolume(dist, mid, dominant, axis1, axis2);
     ...
     */
    private static float analyticalVolume(float[] dist, float offset,
                                                         int dominant, int axis1, int axis2) {
     ...
     */
    private static List<float[]> findPlaneEdgeIntersections(float[][] corners, float[] dist) {
     ...
     */
    private static int alignedGridRes(float[] mins, float[] maxs, int axis1, int axis2) {
     ...
     */
    private static List<float[]> clipFace(float[][] corners, int[] face, float[] dist) {
     ...
     */
    private static float[] stepWaveSimulation(BlockPos key, GridDims grid,
     ...
     */
    private static void buildGrid(GridDims grid, TankBounds bounds, SurfacePlane plane,
     ...
     */
    private static void renderSurface(VertexConsumer vc, Matrix4f mat,
        ...
        for (int g1 = 0; g1 < grid.res(); g1++) {
            for (int g2 = 0; g2 < grid.res(); g2++) {
                ...
                if (outsideCount == 0) {
                    if (flip) {
                        emitQuad(vc, mat, r01, r11, r10, r00, qu0, qu1, qv1, qv0, s);
                    } else {
                        emitQuad(vc, mat, r00, r10, r11, r01, qu0, qu1, qv0, qv1, s);
                    }
                } else {
                    // Boundary cell — clip polygon to tank bounds for smooth diagonal edge
                    emitClippedCell(vc, mat,
    ...
    private static float[] clampToBox(float[] p, float[] mins, float[] maxs) {
        return new float[]{
     ...
     */
    private static void emitClippedCell(VertexConsumer vc, Matrix4f mat,
     ...
     */
    private static void renderWallSkirts(VertexConsumer vc, Matrix4f mat,
                                                        GridDims grid, float[][] gridRaw, boolean[] gridOutside,
        ...
        for (int[] edge : edges) {
            ...
            for (int i = 0; i < res; i++) {
                ...
                float[] tA = clampToBox(sA, mins, maxs);
                float[] tB = clampToBox(sB, mins, maxs);
     ...
     */
    private static void renderWallFaces(VertexConsumer vc, Matrix4f mat,
        ...
        for (int f = 0; f < 6; f++) {
            ...
            List<float[]> clipped = clipFace(corners, face, dist);
     ...
     */
    private static void renderDebug(VertexConsumer lvc, Matrix4f mat,
     ...
     */
    private static void renderDebugWireframe(VertexConsumer lvc, Matrix4f mat,
                                                             GridDims grid, float[][] gridRaw, boolean[] gridOutside,
                                                             TankBounds bounds, SurfacePlane plane,
                                                             float[][] corners, float[] dist) {
        float[] mins = bounds.mins(), maxs = bounds.maxs();
